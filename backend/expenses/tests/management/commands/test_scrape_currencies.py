...
class TestScrapeCurrenciesCommand(TestCase):
    ...
    def test_successful_scraping_when_no_currencies_exist(self):
        ...
        with patch(
            ...
            self.assertIn(f"Fetching exchange rates for {yesterday.isoformat()}...", output)
            self.assertIn(
    ...
    def test_successful_scraping_when_some_currencies_already_exist(self):
        ...
        DollarExchangeRateFactory(currency=self.eur, date=yesterday, rate=Decimal("42.0"))
    ...
    def test_exchange_rate_error_handling(self):
        ...
        with patch(
            "expenses.managers.exchange_rate_manager.get_exchange_rate_from_api",
            return_value=[
                RateResponse(currency_code="EUR", rate=Decimal("0.85"), day=date_utils.today() - dt.timedelta(days=1))
            ]
        ):
            with patch(
    ...
    def test_unexpected_error_handling(self):
        ...
        with patch(
            ...
            output = stderr.getvalue()
            self.assertIn(f"Unexpected error while scraping currencies: {error_message}", output)
