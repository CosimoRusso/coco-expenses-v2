...
class Seeker(Iterator[datetime]):
    ...
    def __init__(
        ...
        if start_date:
            ...
            try:
                ...
                raise ValueError(
                    f"Input schedule start time is not a valid datetime object. Error -> {exc}"
                )
        elif timezone_str:
    ...
    def _shift_month(
        self, cron_month_part: "Part", operation: Literal["add", "subtract"]
    ) -> None:
    ...
    def _shift_day(
        ...
        while (
            self.date.day not in cron_day_part.to_list()
            or iso_to_cron_weekday(self.date.isoweekday())
            not in cron_weekday_part.to_list()
        ):
            if operation == "add":
    ...
    def _shift_hour(
        self, cron_hour_part: "Part", operation: Literal["add", "subtract"]
    ) -> bool:
    ...
    def _shift_minute(
        self, cron_minute_part: "Part", operation: Literal["add", "subtract"]
    ) -> bool:
    ...
    @staticmethod
    def _calc_months(
        date: "datetime", months: int, operation: Literal["add", "subtract"]
    ) -> datetime:
        ...
        if operation == "add":
            return date.replace(
                year=year, month=month, day=1, hour=00, minute=00, second=00
            )
        else:
            # Get the last day of the month
            max_month_day = calendar.monthrange(year, month)[1]
            return date.replace(
                year=year, month=month, day=max_month_day, hour=23, minute=59, second=59
            )
