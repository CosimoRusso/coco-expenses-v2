...
class Part:
    ...
    def __lt__(self, other) -> bool:
        ...
        return self.unit.get("name") == other.unit.get("name") and len(self) < len(
            other
        )
    ...
    def __eq__(self, other) -> bool:
        ...
        if not isinstance(other, Part):
            return NotImplemented
        return (
            self.unit.get("name") == other.unit.get("name")
    ...
    def from_list(self, part_list: List[Union[str, int]]) -> None:
        ...
        unique_values = list(
            dict.fromkeys(sunday_fixed_values)
        )  # Remove eventual duplicates
        unique_values.sort()
        out_of_range_value = self.out_of_range(unique_values)
        if out_of_range_value is not None:
            raise ValueError(
                f"Value {out_of_range_value!r} out of range for {self.unit.get('name')!r}"
            )
    ...
    def from_string(self, cron_part: str) -> None:
        ...
        for string_part in string_parts:
            ...
            if len(range_step_string_parts) > 2:
                raise ValueError(
                    f"Invalid value {string_part!r} in cron part {cron_part!r}"
                )
            range_string = range_step_string_parts[0]
            if not range_string:
                raise ValueError(
                    f"Invalid value {string_part!r} for {self.unit.get('name')!r}"
                )
            elif range_string == "*":
                ...
                if value is not None:
                    raise ValueError(
                        f"Value {value!r} out of range for {self.unit.get('name')!r}"
                    )
            step = self._get_step(range_step_string_parts)

            if step is not None:
                interval_values = self._apply_interval(
                    range_list, step
                )  # filter by step
        ...
        flattened_ranges_list = [
            item for sublist in intervals_values_list for item in sublist
        ]
        flattened_ranges_list = list(
            dict.fromkeys(flattened_ranges_list)
        )  # Remove eventual duplicates
    ...
    def _parse_range(self, unit_range: str) -> List[int]:
        ...
        if len(sub_parts) == 1:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid min or max value from: {unit_range!r} --> {exc}"
                )
            if max_value < min_value:
                raise ValueError(
                    f"Max range is less than min range in {unit_range!r} for {self.unit.get('name')!r}"
                )
    ...
    def _parse_step(self, step: str) -> int:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid interval step value {step!r} for {self.unit.get('name')!r}"
            )
    ...
    def _apply_interval(values: List[int], step: int) -> List[int]:
        ...
        if step:
            ...
            values = [
                value
                for value in values
                if value % step == min_value % step or value == min_value
            ]
        return values
    ...
    def is_full_interval(self, step: int) -> bool:
        ...
        """
        have_all_values = (
            len(self.values) == round((self.max() - self.min()) / step) + 1
        )
        if (
            self.min() == self.unit.get("min")
            and self.max() + step > self.unit.get("max")
            and have_all_values
        ):
            return True
        return False
    ...
    def format_value(self, value: int) -> Union[int, str]:
        ...
        """
        if (
            "output_weekday_names" in self.options
            and self.options.get("output_weekday_names")
            and self.unit.get("name") == "weekday"
            or "output_month_names" in self.options
            and self.options.get("output_month_names")
            and self.unit.get("name") == "month"
        ):
