...
def get_exchange_rate_from_api(day: dt.date) -> DollarExchangeRate:
...
def save_exchange_rates_to_database(rates: dict, day: dt.date):
    available_currencies = Currency.objects.all()
    rates_to_save = []
    for currency in available_currencies:
        rate = rates.get(currency.code)
        if rate:
            rates_to_save.append(DollarExchangeRate(currency=currency, date=day, rate=rate))
